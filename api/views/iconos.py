...
def registerIcono(request):
    ...
    if imagen is None:
        return Response(
            {"error": "La imagen es requerida"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    data = {"nombre": request.data.get("nombre"), "imagen": imagen, "estado": 1}
...
@authentication_classes([CustomJWTAuthentication])
@permission_classes([IsAuthenticated])
def listIconoId(_request, idiconos):
...
def updateIcono(request, idiconos):
    try:
        ...
        return Response(
            {"error": "Icono no encontrado"}, status=status.HTTP_404_NOT_FOUND
        )
...
def deleteIcono(_request, idiconos):
    ...
    icono.save()
    return Response(
        {"message": "Icono desactivado correctamente"}, status=status.HTTP_200_OK
    )
